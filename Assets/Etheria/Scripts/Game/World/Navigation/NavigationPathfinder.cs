...
namespace Etheria.Game.World
{
        ...
        INavigationPathfinder
    {
            ...
            out NavigationPath path)
        {
            ...
            if (!IsTraversableNode(graph, fromNodeId, filter) ||
                !IsTraversableNode(graph, toNodeId, filter))
                return false;
            ...
            while (openSet.Count > 0)
            {
                ...
                foreach (var edge in graph.GetEdges(currentNodeId))
                {
                    ...
                    var nextDistance = currentDistance + edge.Cost;

                    if (distances.TryGetValue(edge.ToNodeId, out var knownDistance) &&
        ...
        private static bool IsTraversableNode(
            ...
            NavigationQueryFilter filter)
        {
            return graph.TryGetNode(nodeId, out var node) &&
            ...
            out string result)
        {
            ...
            {
                if (!distances.TryGetValue(nodeId, out var cost))
                    continue;
            ...
            float totalCost)
        {
            ...
            {
                if (!previous.TryGetValue(currentNodeId, out var previousNodeId))
