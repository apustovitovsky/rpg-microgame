...
namespace Game.Navigation
{
    ...
    {
        private static readonly IReadOnlyList<NavigationEdge>
            EmptyEdges =
                Array.Empty<NavigationEdge>();

        private readonly IReadOnlyDictionary<
            string,
            NavigationNode> _nodesById;

        private readonly IReadOnlyDictionary<
            string,
            IReadOnlyList<NavigationEdge>> _edgesByNodeId;

        public NavigationGraph(
            IReadOnlyDictionary<string, NavigationNode> nodesById,
            IReadOnlyDictionary<
                string,
                IReadOnlyList<NavigationEdge>> edgesByNodeId)
        {
            _nodesById = nodesById
                ?? throw new ArgumentNullException(
                    nameof(nodesById));

            _edgesByNodeId = edgesByNodeId
                ?? throw new ArgumentNullException(
                    nameof(edgesByNodeId));
        }
